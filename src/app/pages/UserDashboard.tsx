...
export default function UserDashboard() {
    ...
    const [bgUrl, setBgUrl] = useState<string>("/bg-images/userdashboardbg.png");

    const [eventLogoUrl, setEventLogoUrl] = useState<string>("");
    const [screenLogoUrl, setScreenLogoUrl] = useState<string>("");

    const [enableCheckout, setEnableCheckout] = useState(true);

    const [checkInTag, setCheckInTag] = useState<{ size: TagSize; bgUrl: string; logoUrl: string }>({
        size: "8x2",
        bgUrl: "/bg-images/previewheaderbg.png",
        logoUrl: "",
    });

    const [checkOutTag, setCheckOutTag] = useState<{ size: TagSize; bgUrl: string; logoUrl: string }>({
        size: "8x2",
    ...
    const [fields, setFields] = useState<GuestField[]>([
    ...
    return (
        <div
            className="w-full absolute top-0 right-0 overflow-x-hidden min-h-screen bg-cover bg-center bg-no-repeat"
            style={bgStyle}
        >
            <div className="max-w-[1280px] flex flex-col justify-between mt-20 sm:mt-28 lg:mt-22 2xl:mt-26 h-[80vh] mx-auto">
                ...
                <UserQuickActions
                    onGuestList={() => setGuestListOpen(true)}
                    onAddWalkingGuest={() => setWalkingGuestOpen(true)}
                    onPrinterStatus={() => console.log("Printer Status")}
                />
            </div>

            <GuestListModal isOpen={guestListOpen} onClose={() => setGuestListOpen(false)} fields={fields} />
