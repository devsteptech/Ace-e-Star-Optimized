...
function mapBackendTemplateToDraft(t) {
    return {
        templateName: t?.templateName ?? "",
        eventType: t?.eventType ?? "Wedding",
        guestFields: Array.isArray(t?.guestFields) && t.guestFields.length
            ? t.guestFields.map((f) => ({
                id: f.id ?? uid(),
                label: f.label ?? "",
...
export const templatesRepo = {
    list() {
        return get("/api/templates", { authMode: "admin" });
    },
    async create(draft) {
        const res = await post("/api/templates", mapDraftToPayload(draft), { authMode: "admin" });
        return res.card;
    },
    async update(id, draft) {
        const res = await put(`/api/templates/${id}`, mapDraftToPayload(draft), { authMode: "admin" });
        return res.card;
    },
    remove(id) {
        return del(`/api/templates/${id}`, { authMode: "admin" });
    },
    async getDraft(id) {
        const doc = await get(`/api/templates/${id}`, { authMode: "admin" });
